...
bool update(UI_NTT& costs, UI_NTT3& pred, UI_NTT& func, UI_NTT& depths, CUI tt, CUI cost, CUS depth, CUS function, CUI ci, CUI cj = INF, CUI ck = INF, const bool verbose = false)
{
    // frontier.insert(tt);
    if (tt == ci || tt == cj || tt == ck) 
    {
        return false;
    }

    bool status = cost < costs[tt];
