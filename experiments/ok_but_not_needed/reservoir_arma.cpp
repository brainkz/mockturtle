...
mockturtle::aig_network generate_ntk()
{
    KLUT ntk;
    ...
    for (auto i = 0u; i < RES_SIZE; i++)
    {
        ...
        if (MAXVARS < 7)
        {
            UL func = std::rand() & ((1 << (1 << MAXVARS)) - 1);
        ...
        {
            auto func_word = generate_random_numbers( (1 << (MAXVARS-6)) );

            kitty::create_from_words( function, func_word.begin(), func_word.end());
        }

        // select input nodes for KLUT
        std::vector<SIGNAL> selected = randomArrayElements( nodes, MAXVARS );

        // create a KLUT gate
        SIGNAL s = ntk.create_node( selected, function );

        // assign PO to KLUT output
        res_output_nodes.push_back(s);
        nodes.push_back(s);
        res_output_index.push_back( ntk.create_po(s) );
    ...
    mockturtle::aig_network res_aig = mockturtle::cleanup_dangling ( mockturtle::convert_klut_to_graph<mockturtle::aig_network>( ntk ) );
    return res_aig;
}

template <typename Ntk>
std::vector<std::vector<bool>> sim_ntk(Ntk res_aig, std::vector<bool> img_vec, UI Nimg, UI Npxl)
{
    ...
    for (auto i = 0u; i < Nimg; i++)
    {
        ...
        mockturtle::default_simulator<kitty::static_truth_table<16u>> sim();
...
int main()
{
...
#if true
    mockturtle::aig_network res_aig = generate_ntk();
    ...
    std::vector<std::vector<bool>> data_out = sim_ntk(res_aig, img_vec, Nimg, Nrow * Ncol); // need to import the MNIST
    ...
    std::cout << "Minimum of squared function found with simulated annealing is " << weights; }
