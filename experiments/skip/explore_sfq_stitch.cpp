...
// void results_old_new(const std::vector<ULL>& hashes_i, const std::vector<ULL>& hashes_j, std::vector<std::vector<UI>>& funcs, std::vector<std::vector<bool>>& xorables, std::vector<std::vector<UI>>& costs, const ULL start_row, const ULL end_row, const UI gate_cost, const ULL offset, const UI tgt_lvl) 
// {
    ...
    else //need to split into chunks
    {
        ...
        if (N < MAX_SIZE / NUM_THREADS)
        {
            ...
            for (ULL start = 0u; start < M; start += Mlocal)
            {
                ...
                {
                    Node & n = GNM[GEX[func]];
                    best_costs[func] = (n.lvl < tgt_lvl) ? INF : n.cost;
                }
...
void threaded_new_new(const std::vector<ULL>& hashes, std::vector<UI>& funcs, std::vector<bool>& xorables, std::vector<UI>& costs, const std::vector<bool>& skip, const std::array<UI, NUM_TT> best_costs, const ULL start_k, const ULL end_k, const UI gate_cost, const ULL offset, const UI tgt_lvl) 
{
    ...
    {   
        COMPUTE_IJ(k, N, i, j);
        const ULL hi = hashes[i]; // fmt::print("\t\t hash_i={}\n", hi);
        ...
        // fmt::print("\t\tk={}, i={}, j={}\n", k, i, j);
        const ULL hj = hashes[j]; // fmt::print("\t\t hash_j={}\n", hj);
...
void threaded_new_new_wrapper(const std::vector<ULL>& new_nodes, const UI depth, std::vector<ULL>& fresh_nodes, UI num_threads = NUM_THREADS)
{
    ...
    else //too many combinations to fit into memory
    {
        ...
        for (auto [START, END]  : divide_range(Ncombs, NUM_MACRO_CHUNKS))
        {
            ...
            std::vector<bool> skip(chunk_size, false);
            std::array<UI, NUM_TT> best_costs;
            for (UI func = 0; func < NUM_TT; ++func)
            {
                Node & n = GNM[GEX[func]];
            ...
            for (auto [start, end] : divide_range(chunk_size, NUM_THREADS))
            {
                ...
                ULL end_k   = end   + START;
                threads.push_back(
                    std::thread(threaded_new_new, std::ref(new_nodes), std::ref(funcs), std::ref(xorables), std::ref(costs), std::ref(skip), best_costs, start_k, end_k, COSTS[fCB], START, tgt_lvl)
                );
            }
            join_threads(threads);
            check_threads(threads);
...
void threaded_and_or(const std::vector<ULL>& hashes, std::vector<UI>& funcs_and, std::vector<UI>& costs_and, std::vector<UI>& funcs_or, std::vector<UI>& costs_or, const ULL start_k, const ULL end_k, const UI gate_cost_and, const ULL offset, const UI tgt_lvl) 
{
    ...
    {   
        ...
        const UI func_and = ni.func & nj.func;
        const ULL best_and_hash = GEX[func_and];
        UL best_and_lvl;
        UL best_and_cost;
        ...
        else
        {
            ...
            // {
            //     fmt::print("best_and_hash : {}\nni : {}\nnj : {}\nfunc_and : {}\nnode in GNM : {}", best_and_hash, ni.to_str(), nj.to_str(), func_and, GNM[best_and_hash].to_str());
            // };
            const Node & best_and = GNM[best_and_hash]; // [best_and_hash];
            best_and_lvl = best_and.lvl;
            best_and_cost = best_and.cost;
        }
        const UI func_or  = ni.func | nj.func;
        const ULL best_or_hash  = GEX[func_or ];
        UL best_or_lvl;
        UL best_or_cost;
        ...
        else
        {
            ...
            // {
            //     fmt::print("best_or_hash : {}\nni : {}\nnj : {}\nfunc_or : {}\nnode in GNM : {}", best_or_hash, ni.to_str(), nj.to_str(), func_or, GNM[best_or_hash].to_str());
            // };
...
std::tuple<ULL, bool> check_cb(const Node& ni, const Node& nj, std::vector<ULL>& fresh_nodes)
{
    // fmt::print("\t\tA: {}\n", ni.to_str());
    // fmt::print("\t\tB: {}\n", nj.to_str());
    const UI func = ni.func | nj.func;
...
std::tuple<ULL, bool> check_xor(const Node& ni, const Node& nj)
{
    // fmt::print("\t\tA: {}\n", ni.to_str());
    // fmt::print("\t\tB: {}\n", nj.to_str());
    const UI func = ni.func ^ nj.func;
...
std::tuple<ULL, bool, ULL, bool> check_and_or(const Node& ni, const Node& nj)
{
    // fmt::print("\t\tA: {}\n", ni.to_str());
    // fmt::print("\t\tB: {}\n", nj.to_str());
    US depth = ni.lvl * 3 + 3;
    ...
    // if (added_and)
    // {
    //     const Node& n =  GNM[hash_and];
    //     fmt::print("\t\t&: {}\n", n .to_str());
    // }
    // if (added_or )
    // {
    //     const Node& n =  GNM[hash_or ];
    //     fmt::print("\t\t|: {}\n", n .to_str());
    // }
    ...
    do {
        ...
        // write_csv_arr(GEX, fmt::format("{}_gex_cb_{}_{}_{}.csv", level_prefix, lvl, iteration, get_current_time_formatted()));
            ...
            for (ULL k = 0u; k < new_nodes.size() * old_nodes.size(); k++)
            {
                ...
                fmt::print("\tCB ({}, {}, {}):\n", i, j, k);
                const Node& ni =  GNM[new_nodes[i]];
                const Node& nj =  GNM[old_nodes[j]];
        ...
        new_nodes.clear();
        for (const ULL fresh_hash : fresh_nodes)
        {
...
void as_generation(US lvl)
{
    ...
    fmt::print("\t{}: XOR {:L} nodes\n", lvl, nodes.size());
        ...
        for (ULL k = 0u, n = nodes.size(); k < n * (n - 1) / 2; k++)
        {
            COMPUTE_IJ(k, n, i, j);
            fmt::print("\tXOR ({}, {}, {}):\n", i, j, k);
            const Node& ni =  GNM[nodes[i]];
            const Node& nj =  GNM[nodes[j]];
    ...
    for (std::vector<UI> levels : sets_of_levels) 
    {
        ...
        for (const auto & [hash, n1] : GNM)
        {
            ...
            if (!(scalar_fields_equal && parents_equal))
            {
                ...
                for (const ULL phash : n1.parent_hashes)
                {
                    fmt::print("\t\tParents 1\t: {}\n", phash);
                }
                for (const ULL phash : n2.parent_hashes)
                {
                    fmt::print("\t\tParents 2\t: {}\n", phash);
                }
            }
...
/*
nohup ./experiments/explore_sfq_vec > 20230328_vec/x3.log 2>&1 &
*/