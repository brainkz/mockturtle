...
void splitter_ntk_insertion(klut & ntk, const bool verbose = false)
{
  ...
  ntk_fo.foreach_node([&](const klut::signal & node)
  {
    ...
    {
      DEBUG_PRINT("\t\t\t node: {}, σ={}\n", node, NodeData(ntk.value(node)).sigma);
      
      fo_ntk.foreach_fanin( node, [&](const klut::signal & fi_node)
      {
        if (ntk_fo.is_dangling(fi_node))
        {
          return;
        }
        DEBUG_PRINT("\t\t\t\t fanin : {}, σ={}\n", fi_node, NodeData(ntk.value(fi_node)).sigma);
      });
      fo_ntk.foreach_fanout( node, [&](const klut::signal & fo_node)
      {
        ...
        DEBUG_PRINT("\t\t\t\t fanout: {}, σ={}\n", fo_node, NodeData(ntk.value(fo_node)).sigma);
      });
    }
...
{
  return ( static_cast<uint64_t>(_fanin) << 40 ) | ( static_cast<uint64_t>(_fanout) << 16 ) | _sigma;
}
